...
// Get all applicants from rejected_applicants_tbl
$sql = "SELECT * FROM rejected_applicants_tbl";
...
if ($result->num_rows > 0) {
    while ($applicant = $result->fetch_assoc()) {
        ...
        $maxSql = "SELECT MAX(applicant_id) FROM rejected_applicants_tbl";
        $resultMax = $conn->query($maxSql);
        if ($resultMax) {
            $row = $resultMax->fetch_row();
            $newAutoIncrement = $row[0] + 1;
            $resetSql = "ALTER TABLE rejected_applicants_tbl AUTO_INCREMENT = $newAutoIncrement";
            $conn->query($resetSql);
        }
    ...
    echo "<script>
        window.onload = function() {
            Swal.fire({
                icon: 'success',
                title: 'All Rejected Applicants Removed',
                text: 'All rejected applicants have been removed.',
    ...
    echo "<script>
        window.onload = function() {
            Swal.fire({
                title: 'Error!',
                text: 'No rejected applicants found.',
                icon: 'error',
                confirmButtonText: 'OK'
            }).then(() => {
                window.location.href = 'rejectedapplicants.php';

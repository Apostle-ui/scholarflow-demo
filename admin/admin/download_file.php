...
// Get applicant_id from GET request
$applicant_id = isset($_GET['applicant_id']) ? $_GET['applicant_id'] : null;

if ($applicant_id === null) {
    die("Applicant ID is missing.");
}

// Fetch the applicant's information for the selected applicant
...
$pdf_info_filename = $temp_dir . $applicant['firstname'] . '_' . $applicant['lastname'] . '_info.pdf';
$pdf = new FPDF();
$pdf->AddPage();
...
// Create a ZIP file for the selected applicant
$zip_filename = '../../php/uploads/' . $applicant['firstname'] . " " . $applicant['lastname'] . ".zip";
if ($zip->open($zip_filename, ZipArchive::CREATE) === TRUE) {
    ...
    unlink($zip_filename); // Delete the applicant's ZIP file
    ...
    die("Failed to create ZIP file for the applicant.");
}

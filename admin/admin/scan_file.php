...
// Get the applicant_id from POST
...
if (!file_exists($school_identification_path)) {
    die("School identification file not found.");
}

if (!file_exists($certificate_registration_path)) {
    die("Certificate registration file not found.");
}
...
if (in_array($fileTypeSchool, ['pdf', 'jpg', 'jpeg', 'png', 'gif', 'bmp', 'docx'])) {
    if ($fileTypeSchool == 'pdf') {
        ...
        try {
            ...
            die("Error parsing school identification PDF: " . $e->getMessage());
        }
...
if ($fileTypeCert === 'pdf') {
    ...
    die("Unsupported certificate registration file type.");
}
    ...
    <script>
        ...
        if (status === 'eligible') {
            Swal.fire({
                ...
                text: 'The address "Alabang, Muntinlupa City" was found and Certificate of Registration is Valid. Applicant has been accepted.',
                confirmButtonText: 'OK'
            }).then(() => {
                window.location.href = 'applicants.php'; // Redirect to applicants page after clicking OK
            });
        } else if (status === 'rejected') {
            Swal.fire({
                icon: 'error',
                title: 'Applicant Rejected',
                text: 'The address "Alabang, Muntinlupa City" was not found or the certificate registration did not match the reference. Applicant has been rejected.',
                confirmButtonText: 'OK'
            }).then(() => {
                window.location.href = 'applicants.php'; // Redirect to applicants page after clicking OK
            });
